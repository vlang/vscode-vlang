...
export function colorSyntax(root: Parser.Tree, visibleRanges: { start: number, end: number }[]) {
	...
	function scanConstDeclaration(x: Parser.SyntaxNode) {
		for (const constSpec of x.namedChildren) {
			for (const child of constSpec.namedChildren) {
				switch (child.type) {
					case 'identifier':
						if (isVisible(child, visibleRanges)) {
